...
export default function DetailModal({ item, type, onClose, onUpdate }) {
  ...
  useEffect(() => {
    if (tomasOpen && !type || tomasOpen && type !== 'champagne') {
      loadTomas()
    }
  }, [tomasOpen])
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Sección Tomas — solo para vinos, no champagnes */}
        {!isChamp && (
          <div className="mt-6" style={{ borderTop: '1px solid ' + c.accent + '22' }}>
            ...
            >
              <span className="text-[12px] uppercase tracking-wider font-semibold" style={{ color: c.accent }}>
                📅 Historial de tomas {tomas.length > 0 && !tomasLoading ? `(${tomas.length})` : ''}
              </span>
            ...
            {tomasOpen && (
              <div className="pb-2">
                ...
                {tomasLoading ? (
                  ...
                  <p className="text-[#555] text-sm italic">Aún no hay tomas registradas.</p>
                ) : (
